...
function getSortIndicator(sorted: false | 'asc' | 'desc'): string {
  if (sorted === 'asc') return ' \u25B2'
  if (sorted === 'desc') return ' \u25BC'
  return ''
}

function IssueHeaderCellFilter({
  column,
  statusOptions,
}: {
  column: Column<Issue>
...
export function IssueHeaderCell({
  header,
  statusOptions,
}: {
  header: Header<Issue, unknown>
  statusOptions: MultiSelectOption[]
}): ReactElement {
  return (
