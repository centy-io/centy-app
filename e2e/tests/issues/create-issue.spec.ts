...
test.describe('Create Issue Form', () => {
  test.describe('Page rendering', () => {
    ...
    test('should render the priority select with all options', async ({
      ...
      await expect(
        prioritySelect.locator('option[value="1"]')
      ).toContainText('High')
      await expect(
        prioritySelect.locator('option[value="2"]')
      ).toContainText('Medium')
      await expect(
        prioritySelect.locator('option[value="3"]')
      ).toContainText('Low')
    })
    ...
    test('should render form group labels', async ({ page }) => {
      ...
      await expect(page.locator('.form-label').filter({ hasText: 'Title' })).toBeVisible()
      await expect(page.locator('.form-label').filter({ hasText: 'Description' })).toBeVisible()
      await expect(page.locator('.form-label').filter({ hasText: 'Priority' })).toBeVisible()
      await expect(page.locator('.form-label').filter({ hasText: 'Status' })).toBeVisible()
    })
  })
  ...
  test.describe('Form interactions', () => {
    ...
    test('should populate status select with demo config allowed states', async ({
      ...
      await expect(statusSelect.locator('option[value="in-progress"]')).toBeAttached()
      await expect(statusSelect.locator('option[value="for-validation"]')).toBeAttached()
    ...
    test('should restore draft title from localStorage on mount', async ({
      ...
      await page.addInitScript((key: string) => {
        localStorage.setItem(
          key,
          JSON.stringify({ title: 'Restored issue', description: '', priority: 2 })
        )
      }, DRAFT_STORAGE_KEY)
  ...
  test.describe('Successful submission', () => {
    ...
    test('should clear the draft from localStorage after successful submission', async ({
      ...
      await page.addInitScript((key: string) => {
        localStorage.setItem(
          key,
          JSON.stringify({ title: 'Will be cleared', description: '', priority: 2 })
        )
      }, DRAFT_STORAGE_KEY)
